...
func getTupleID(pkt *PacketBuffer) (tid tupleID, allowNewConn bool, ok bool) {
	...
	case header.ICMPv4ProtocolNumber:
		...
		case header.ICMPv4Echo:
			return getTupleIDForEchoPacket(pkt, icmp.Ident(), true /* request */), true, true
		case header.ICMPv4EchoReply:
			// Do not create a new connection in response to a reply packet.
			return getTupleIDForEchoPacket(pkt, icmp.Ident(), false /* request */), false, true
		case header.ICMPv4DstUnreachable, header.ICMPv4TimeExceeded, header.ICMPv4ParamProblem:
		default:
...
func (cn *conn) performNAT(pkt *PacketBuffer, hook Hook, r *Route, portsOrIdents portOrIdentRange, natAddress tcpip.Address, dnat bool) {
	...
	*address = natAddress
	...
	if end := portsOrIdents.start + portsOrIdents.size - 1; *portOrIdent >= portsOrIdents.start && *portOrIdent <= end {
	...
	for maxAttempts := allowedInitialAttempts; ; maxAttempts /= 2 {
		// Start reach round with a random initial port/ident in the range.
		initial := portsOrIdents.start + uint16(cn.ct.rand.Uint32())%portsOrIdents.size

		for i := uint16(0); i < maxAttempts; i++ {
			*portOrIdent = initial + i%portsOrIdents.size
