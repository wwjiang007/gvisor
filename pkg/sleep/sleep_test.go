...
func TestAssertFetch(t *testing.T) {
	...
	// Fire a random set of assertion. This should not cause any problems
	// with the ring, since every fetch also does at least one assertion.
	// These may just be eaten (or eat the above assertions). This ring
	// should collapse into execution on a single P.
	start := rand.Int()
	for i := 0; i < seedAsserts; i++ {
		ws[(start+i)%sleeperWakers].Assert()
	}
	wg.Wait()
...
// named benchmark, except it uses the sychronous AssertAndFetch.
