...
func (f *overlayFileOperations) EventRegister(we *waiter.Entry, mask waiter.EventMask) error {
	f.upperMu.Lock()
	defer f.upperMu.Unlock()
	if f.upper != nil {
		return f.upper.EventRegister(we, mask)
	}
	return f.lower.EventRegister(we, mask)
}
